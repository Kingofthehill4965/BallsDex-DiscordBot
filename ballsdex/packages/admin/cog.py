...
class Admin(commands.GroupCog):
    ...
    async def cooldown(
        ...
        if delta < 600:
            informations.append(
                f"The manager is less than 10 minutes old, {setting.plural_collectible_name} "
    ...
    async def _spawn_bomb(
        ...
        try:
            ...
            await interaction.followup.edit_message(
                "@original",  # type: ignore
                content=f"Successfully spawned {spawned} {setting.plural_collectible_name} "
    ...
    async def balls_reset(
        ...
        if not percentage:
            text = f"Are you sure you want to delete {user}'s {setting.plural_collectible_name}?"
        else:
            text = (
                f"Are you sure you want to delete {percentage}% of "
                f"{user}'s {setting.plural_collectible_name}?"
            )
        ...
        await interaction.followup.send(
            f"{count} {setting.plural_collectible_name} from {user} have been reset.", ephemeral=True
        )
        await log_action(
            f"{interaction.user} deleted {percentage or 100}% of "
            f"{player}'s {setting.plural_collectible_name}.",
            self.bot,
        )
    ...
    async def user(
        ...
        embed.add_field(
            name=f"Total Server with {setting.plural_collectible_name} caught ({days} days))",
            value=len(set([x.server_id for x in total_user_balls])),
        )
        embed.add_field(
            name=f"Total {setting.plural_collectible_name} Caught",
            value=await BallInstance.filter(player__discord_id=user.id).count(),
        )
        embed.add_field(
            name=f"Total Unique {setting.plural_collectible_name} Caught",
            value=len(set([x.countryball for x in total_user_balls])),
        )
        embed.add_field(
            name=f"Total Server with {setting.plural_collectible_name} Caught",
            value=len(set([x.server_id for x in total_user_balls])),
        )
